"""
Difficulty Rating Prediction using 4 features
...
# ---------- Numeric feature helpers: readability ----------
...
def _compute_readability_features(texts):
    """
    texts: iterable of question stems
...
def _numeric_feats_from_df(X):
    """
    Accepts a DataFrame with a column question_stem or a Series
    and returns readability features as a dense array.
    """
    if isinstance(X, pd.DataFrame):
        stems = X["question_stem"].fillna("").to_numpy()
    else:
        stems = pd.Series(X).fillna("").to_numpy()
...
def build_preprocessor() -> ColumnTransformer:
    ...
    - Readability features from question_stem with scaling
    """
    ...
    num_branch = Pipeline(steps=[
        ...
        # with_mean False is safe if the global matrix ends up sparse
...
MODEL_REGISTRY = {
    # Ridge and LinearSVR do not use random_state
...
def main():
    ...
    # ID based split using question_base_id
    ...
    # Preprocessor with readability branch
    ...
    for name, base_model in MODEL_REGISTRY.items():
        ...
        # Train on full TRAIN and evaluate on frozen TEST
    ...
    lb = pd.DataFrame(leaderboard).sort_values(
        by=["cv_MAE_mean", "cv_RMSE_mean", "cv_R2_mean"], ascending=[True, True, False]
    )
