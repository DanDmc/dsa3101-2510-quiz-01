...
def process_json_files():
    ...
    if target_base:
        json_file = f"{target_base}.json"
        json_path = os.path.join(JSON_DIR, json_file)
        # if not json_path.exists():
            # raise FileNotFoundError(f"TARGET_BASE={target_base} not found in {JSON_DIR}")

        pdf_name = f"{target_base}.pdf"
        ...
        if not content:
            print(f"⚠️ Skipping {json_path} (empty file)\n")
            return
