...
def process_json_files():
    ...
    # --- FIX: Determine which JSON files to process ---
    json_files = []
    if target_base and pipeline_mode_file_id:
        # SINGLE FILE MODE (from app.py)
        json_file_name = f"{target_base}.json"
        
        if not os.path.exists(os.path.join(JSON_DIR, json_file_name)):
            print(f" Error: Target JSON file not found: {json_file_name}")
            cursor.close()
            conn.close()
    ...
    for json_file in json_files:
        ...
        # Note: pdf_name is only used for legacy mode file_id lookup
